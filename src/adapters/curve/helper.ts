...
export async function getCurveBalances(ctx: BaseContext, chain: Chain, contracts: Contract[], registry?: Contract) {
  ...
  try {
    ...
    for (let i = 0; i < nonEmptyPools.length; i++) {
      ...
      nonEmptyPools[i].underlyings = await getERC20Details(chain, nonEmptyPools[i].underlyings as any)

      /**
       *  Updating pool amounts from the fraction of each underlyings
       */

      const formattedUnderlyings = nonEmptyPools[i].underlyings?.map((underlying, x) => ({
        ...underlying,
        amount: underlying.decimals && nonEmptyPools[i].amount.mul(underlyingsBalances[x]).div(totalSupply),
