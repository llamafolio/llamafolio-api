...
export async function getStakeBalances(ctx: BaseContext, chain: Chain, contracts: Contract[]): Promise<Balance[]> {
  ...
  for (let i = 0; i < contracts.length; i++) {
    ...
    if (contract.name === 'sGLP') {
      ...
      const GMXStaker = new ethers.Contract('0x4e971a87900b931ff39d1aad67697f49835400b6', GMXStakerAbi, provider)
    ...
    if (contract.name === 'sGMX') {
      const GMXStaker = new ethers.Contract(contract.address, GMXStakerAbi, provider)
      ...
      const GMXStakerFees = new ethers.Contract('0xd2D1162512F927a7e282Ef43a362659E4F2a728F', GMXStakerAbi, provider)
