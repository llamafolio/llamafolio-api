...
export async function getGaugesBalances(
  ...
  for (let i = 0; i < getGaugesBalances.length; i++) {
    ...
    const claimableCurveToken = claimableCurveTokens[i]
    const reward = registry.rewards?.[0]

    balances.push({
      ...gauge,
      poolAddress: nonEmptyPool.poolAddress,
      lpToken: nonEmptyPool.lpToken,
      rewards: [{ ...(reward as Balance), amount: claimableCurveToken }],
      yieldKey: nonEmptyPool.lpToken,
      category: 'farm',
    })
  }
