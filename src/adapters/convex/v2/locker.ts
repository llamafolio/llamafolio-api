...
export async function getLockerBalances(
  ...
  try {
    ...
    const balanceLocked = balanceLockedRes.output[3].map((balance: any) =>
      BigNumber.from(balance.amount)
    );
    const formattedBalanceLocked: BigNumber = balanceLocked.reduce(
      (previous: BigNumber, current: BigNumber) => previous.add(current)
    );

    const lockTimeDuration = balanceLockedRes.output[3].map(
      (balance: any) => balance.unlockTime
    );
    const lockEnd = lockTimeDuration.sort(
    ...
    const claimableRewardsBalances = claimableRewards.map((token) =>
      BigNumber.from(token.amount)
    );
