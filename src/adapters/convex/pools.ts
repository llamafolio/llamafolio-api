...
export async function getAllPools() {
  ...
  const booster = new ethers.Contract('0xf403c135812408bfbe8713b5a23a04b3d48aae31', BoosterAbi, provider)
  ...
  const tokens = poolInfo.map((r) => r.lptoken)
...
export async function getPoolBalances(ctx: BaseContext, chain: string, contracts: any[]) {
  ...
  const balancesR = balancesRes.filter((res) => res.success).map((res) => res.output)
  ...
  const earnedR = earnedRes.filter((res) => res.success).map((res) => res.output)
  ...
  const extraRewards = extraRewardsRes.filter((res) => res.success).map((res) => res.output)
