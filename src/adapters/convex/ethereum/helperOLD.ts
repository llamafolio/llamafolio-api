...
export async function getCVXRatio(chain: Chain, contract?: Contract, earnedBalances?: BigNumber) {
...
export async function getPoolsContractsFromLpTokens(chain: Chain, lpToken?: Contract) {
...
export async function getUnderlyingsRewardsBalances(chain: Chain, contracts: Contract[]) {
  ...
  for (let i = 0; i < contracts.length; i++) {
    ...
    const formattedUnderlyings = contract.underlyings?.map((underlying, x) => ({
      ...underlying,
      amount: underlying.decimals && contract.amount.mul(underlyingsBalances[x]).div(totalSupply),
