...
import { bufToStr, isHex, strToBuf } from '@lib/buf'
...
export const handler: APIGatewayProxyHandler = async (event, context) => {
  ...
  try {
    ...
    const tokenTransfersRes = await client.query(
      format('select distinct on (chain, tt_transaction_hash, tt_log_index) t.* from ( %s ) as t', tokenTransfersQuery),
    )
    ...
    for (const row of tokenTransfersRes.rows) {
      ...
      transactionByChainHash[`${chain}:${hash}`].token_transfers.push({
        ...
        decimals: row.tt_decimals != null ? parseInt(row.tt_decimals) : row.tt_decimals,
