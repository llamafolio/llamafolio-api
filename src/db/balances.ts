...
export function toStorage(balances: PricedBalance[], adapterId: string, fromAddress: string, timestamp: Date) {
  ...
  for (const balance of balances) {
    ...
    const c = {
      ...
      price_timestamp: priceTimestamp ? priceTimestamp.toString() : undefined,
    ...
    if (rewards && rewards.length > 0) {
      for (const reward of rewards) {
        res.push({
          ...
          price_timestamp: reward.timestamp ? reward.timestamp.toString() : undefined,
          amount: reward.amount.toString(),
          balance_usd: reward.balanceUSD.toString(),
          timestamp: timestamp.toString(),
          parent: c.address,
          // data: {
          //   claimable,
    ...
    if (underlyings && underlyings.length > 0) {
      for (const underlying of underlyings) {
        res.push({
          ...
          price: underlying.price.toString(),
          price_timestamp: underlying.timestamp ? underlying.timestamp.toString() : undefined,
          amount: underlying.amount.toString(),
          balance_usd: underlying.balanceUSD.toString(),
          timestamp: timestamp.toString(),
          parent: c.address,
        })
      }
