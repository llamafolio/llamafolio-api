...
import { selectAdaptersContractsByAddress, selectAdaptersContractsByAddressV1 } from '@db/contracts'
...
export const handlerV1: APIGatewayProxyHandler = async (event, context) => {
  const chain = event.pathParameters?.chain as Chain
  const address = event.pathParameters?.address?.toLowerCase() as `0x${string}`
  ...
  try {
    ...
    // `token` key can also be used to retrieve token details, ignore it
    const adaptersContracts = (await selectAdaptersContractsByAddressV1(client, address, chain)).filter(
      (contract) => !contract.token || contract.token?.toLowerCase() === address,
    )
