...
export async function getSolidlizardContracts(ctx: BaseContext, voter: Contract): Promise<Contract[]> {
  const poolLength = await call({ ctx, target: voter.address, abi: abi.poolsLength })
