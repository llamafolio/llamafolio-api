...
export async function getIFOBalances(ctx, chain) {
  ...
  for (let index = 0; index < poolBalances.length; index++) {
    ...
    if (poolBalance > 0) {
      ...
      const pendingReward = pendingRewards.find((o) => o.input.params[0] === correspondingKeys[poolDetails[index].lpToken]);
