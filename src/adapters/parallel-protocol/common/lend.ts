...
const PARbyChain: { [key: string]: Contract } = {
  ethereum: PAR,
  polygon: replaceAddress('polygon', '0xe2aa7db6da1dae97c5f5c6914d285fbfcc32a128'),
  fantom: replaceAddress('fantom', '0x13082681e8ce9bd0af505912d306403592490fc7'),
}

function replaceAddress(chain: Chain, address: `0x${string}`) {
  return { ...PAR, chain, address }
}

export async function getParallelLendBalances(ctx: BalancesContext, vault: Contract, config: Contract) {
  if (!vault) return []

  const assets = vault.tokens as Contract[]
  if (!assets) return []
  ...
  const userVaultId = await multicall({
    ctx,
    calls: assets.map((asset) => ({ target: vault.vault, params: [asset.address, ctx.address] }) as const),
    abi: abi.vaultId,
  })
  ...
  const [userCollateralsRes, userDebtsRes] = await Promise.all([
    multicall({
      ctx,
      calls: userVaults.map((userVault) => ({ target: vault.vault, params: [userVault.vaultId] }) as const),
