...
// Underlying is the same for multichain
const WPC: Token = {
  chain: 'ethereum',
...
export async function getMarketsRewards(ctx: BaseContext, chain: Chain, contract?: Contract): Promise<Balance[]> {
  if (!contract) {
    console.log('Missing or incorrect reward contract')
  ...
  try {
    ...
    console.log('Failed to get markets rewards')
