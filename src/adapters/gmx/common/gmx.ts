...
export async function getGMXContracts(chain: Chain, contract?: Contract) {
  ...
  try {
    const [stakerGmxTrackerRes, gmxRes, wethRes, stakerGmxFeesRes, esGmxRes] =
      await Promise.all([
        call({
          chain,
    ...
    const [stakerGmxTracker, gmx, weth, stakerGmxFees, esGmx] =
      await Promise.all([
