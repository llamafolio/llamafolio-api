...
const updateProtocolsTokens: APIGatewayProxyHandler = async () => {
  await invokeLambda('updateProtocolsTokens', {}, 'Event')

  return success({})
}

/**
 * Insert missing protocols tokens
 */
export const scheduledUpdateProtocolsTokens = wrapScheduledLambda(updateProtocolsTokens)
...
export const handler: APIGatewayProxyHandler = async () => {
  try {
    ...
    console.log('Failed to update protocols tokens', e)
    return serverError(`Failed to update protocols tokens: ${e}`)
  }
}
