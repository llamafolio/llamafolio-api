...
export async function getGaugeBalances(ctx: BaseContext, chain: Chain, gauges: Contract[]) {
  ...
  for (let i = 0; i < gauges.length; i++) {
    ...
    // Underlyings
    if (stakedAmount.gt(0) && balance.underlyings?.length > 0 && totalSupply.gt(0)) {
      ...
      if (underlyingBalances.length === balance.underlyings.length) {
        for (let i = 0; i < balance.underlyings?.length; i++) {
          balance.underlyings[i].amount = stakedAmount.mul(underlyingBalances[i].amount).div(totalSupply)
        }
      }
    }
