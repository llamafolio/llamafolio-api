...
export async function getPoolsContracts() {
  ...
  const registriesList = registriesListRes.filter((res) => res.success).map((res) => res.output)
  ...
  const registriesPoolCount = registriesPoolCountRes.filter((res) => res.success).map((res) => res.output)
  ...
  for (let i = 0; i < mainRegistryPoolsList.length; i++) {
    if (!mainPoolsDetailsNamesRes[i].success || !mainPoolLPTokensRes[i].success) {
  ...
  const underlyingsDetails = await getERC20Details('ethereum', underlyingCoinsAddresses)
...
export async function getPoolsBalances(ctx: BaseContext, chain: Chain, contracts: Contract[]) {
  ...
  const nonEmptyPools = (await getERC20BalanceOf(ctx, chain, contracts)).filter((pool) => pool.amount.gt(0))
  ...
  for (let i = 0; i < underlyingsBalances.length; i++) {
    if (underlyingsBalances[i].success) {
      ...
      const underlyingAddress = underlyingsBalances[i].input.target.toLowerCase()

      const pool = nonEmptyPools.find((pool) => pool.poolAddress === poolAddress)

      if (pool) {
        const underlying = pool.underlyings?.find(
          (underlying: Contract) => underlying.address.toLowerCase() === underlyingAddress,
        )
  ...
  for (let i = 0; i < nonEmptyPools.length; i++) {
    if (
      ...
      nonEmptyPools[i].underlyings.every((underlying) => underlying.amount?.gt(0))
    ) {
      const totalSupply = BigNumber.from(totalSupplyRes[i].output)

      for (const underlying of nonEmptyPools[i].underlyings) {
        underlying.amount = nonEmptyPools[i].amount.mul(underlying.amount).div(totalSupply)
      }
