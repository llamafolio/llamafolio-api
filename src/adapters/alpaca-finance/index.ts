...
import { getDepositBalances, getFarmingBalances } from './balances'
import { getContractsInfos, getPoolsContracts } from './pool'
...
const getBalances: GetBalancesHandler<typeof getContracts> = async (ctx, { contractsBSC, contractsFTM }) => {
  const [farmingbalancesBSC, depositBalancesBSC, farmingbalancesFTM, depositBalancesFTM] = await Promise.all([
  ...
  const balances = [...farmingbalancesBSC, ...depositBalancesBSC, ...farmingbalancesFTM, ...depositBalancesFTM]
